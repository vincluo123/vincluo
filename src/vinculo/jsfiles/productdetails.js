...
import productDetailsData from '../jsonfiles/productdetails.json';
...
const ProductDetails = () => {
  const { id } = useParams(); // Assuming the product ID is passed via URL
  const product = productDetailsData.products.find(p => p.id === parseInt(id)); // Adjust according to your data structure
  ...
  useEffect(() => {
    setSelectedPrice(product.price.blue);
